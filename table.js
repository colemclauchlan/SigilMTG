...
(function () {
  ...
  function buildExtras() {
    ...
    el.stack = st;
    var _lx = document.getElementById("tblLogExpand"); if (_lx) { try { _lx.innerHTML = (window.MTGIcons ? MTGIcons.get("expand", "1em") : ""); } catch (e) {} _lx.onclick = openLogModal; }
  ...
  function doPassTurn() {
    ...
    if (!online) {
      ...
      var drew = false;
      if (autoTurn) { dispatch({ t: "untap_all", seat: as }); if (state.turn > 1 && MTGCore.zoneCount(state, as, "library") > 0) { dispatch({ t: "draw", seat: as, count: 1 }); drew = true; } }
      dispatch({ t: "set_phase", phase: autoTurn ? "main1" : "untap" });
      log("<b>Turn " + state.turn + "</b> — " + (autoTurn ? ("untapped" + (drew ? " &amp; drew for turn." : ".")) : "your turn (manual untap/draw)."));
  ...
  function checkTurnChange() {
    ...
    setTimeout(function () {
      ...
      if (mine && online) {
        var drew = false;
        if (autoTurn) { dispatch({ t: "untap_all", seat: mySeat }); if (state.turn > 1 && MTGCore.zoneCount(state, mySeat, "library") > 0) { dispatch({ t: "draw", seat: mySeat, count: 1 }); drew = true; } }
        dispatch({ t: "set_phase", phase: autoTurn ? "main1" : "untap" });
        log("<b>Turn " + state.turn + "</b> — " + (autoTurn ? ("your turn: untapped" + (drew ? " &amp; drew for turn." : ".")) : "your turn (manual untap/draw)."));
      }
    }, 0);
  }
  ...
  function parseLobbyName(name) { name = String(name || ""); var lfp = /\s*\[LFP\]\s*$/i.test(name); if (lfp) name = name.replace(/\s*\[LFP\]\s*$/i, ""); var m = name.match(/^B([1-5])\s*·\s*(.*)$/); var r = m ? { bracket: +m[1], title: m[2] || "Commander table" } : { bracket: null, title: name || "Commander table" }; r.lfp = lfp; return r; }
  function openLobby() {
    ...
    function wire() {
      ...
      if (hb) hb.onclick = function () {
        ...
        ov.remove();
        var lfpCb = panel.querySelector(".lob-lfp-cb");
        doHost({ visibility: pub ? "public" : "private", bracket: br, scheduledAt: schedIso, lfp: !!(lfpCb && lfpCb.checked) });
      };
  ...
  // Expand the action log into a roomy, scrollable modal, grouped by turn (oldest -> newest).
  function openLogModal() {
    if (!el.log) return;
    var rows = Array.prototype.slice.call(el.log.querySelectorAll(".row")).reverse(); // oldest-first
    var ov = document.createElement("div"); ov.className = "tbl-pileview";
    ov.addEventListener("pointerdown", function (e) { if (e.target === ov) ov.remove(); });
    var panel = document.createElement("div"); panel.className = "pv-panel log-modal";
    var h = '<div class="pv-head"><span class="pv-title">Action log <span class="pv-count">' + rows.length + '</span></span><button class="pv-x pv-x-ic" title="Close" aria-label="Close">' + (window.MTGIcons ? MTGIcons.get("close", "1em") : "close") + '</button></div><div class="log-modal-body">';
    if (!rows.length) h += '<div class="pv-emptymsg">No actions yet.</div>';
    var lastTurn = null;
    rows.forEach(function (r) {
      var tsEl = r.querySelector(".log-ts"), turn = "?";
      if (tsEl) { var m = /T(\d+)/.exec(tsEl.textContent || ""); if (m) turn = m[1]; }
      if (turn !== lastTurn) { h += '<div class="log-turn-sep">Turn ' + esc(turn) + '</div>'; lastTurn = turn; }
      h += '<div class="log-modal-row">' + r.innerHTML + '</div>';
    });
    h += '</div>';
    panel.innerHTML = h;
    panel.querySelector(".pv-x").onclick = function () { ov.remove(); };
  ...
  window.MTGTable = {
    ...
    clearMana: function () { manaPool = { W: 0, U: 0, B: 0, R: 0, G: 0, C: 0 }; try { renderVitals(); } catch (e) {} },
    keepManaOn: function () { return !!keepMana; },
    toggleKeepMana: function () { keepMana = !keepMana; try { renderVitals(); } catch (e) {} return keepMana; },
    commanderTax: function (seat) { var s = (seat == null ? mySeat : seat), t = 0, has = false; try { if (state) for (var id in state.cards) { var c = state.cards[id]; if (c && c.isCommander && (c.controllerSeat != null ? c.controllerSeat : c.ownerSeat) === s) { has = true; t += (c.counters && c.counters.tax) || 0; } } } catch (e) {} return has ? t : null; },
