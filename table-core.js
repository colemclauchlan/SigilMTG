...
(function (root, factory) {
  ...
  function zoneCount(state, seat, zone) { var n = 0; for (var id in state.cards) { var c = state.cards[id]; if (c.ownerSeat === seat && c.zone === zone) n++; } return n; } // direct count: no array alloc/sort (same predicate as cardsOf)
  ...
  function reduce(state, action) {
    ...
    // Composite action: apply sub-actions in order as one undoable/syncable unit
    // (e.g. commander damage + the matching life loss — see invert("batch")).
    if (action.t === "batch") {
      var bs = state;
      (action.actions || []).forEach(function (sub) { bs = reduce(bs, sub); });
      return bs;
    }
    var s = clone(state);
    var a = action;
    switch (a.t) {
      ...
      case "untap_all": { for (var id in s.cards) { var c = s.cards[id]; if (c.controllerSeat === a.seat && c.zone === "battlefield" && !c.phased) { c.tapped = false; if (c.attacking) c.attacking = false; } } return s; }
      case "card_counter": {
      ...
      case "commander_damage": {
        ...
        var dnv = Math.max(0, (dp.cmdDamage[key] || 0) + (a.delta || 0)); // damage totals never go negative
  ...
  function invert(action, before) {
    ...
    if (a.t === "batch") {
      var st = before, invs = [];
      (a.actions || []).forEach(function (sub) { invs.push(invert(sub, st)); st = reduce(st, sub); });
      invs.reverse();
      return { t: "batch", actions: invs };
    }
    switch (a.t) {
      ...
      case "untap_all": { var aff = []; for (var id in before.cards) { var c2 = before.cards[id]; if (c2.controllerSeat === a.seat && c2.zone === "battlefield" && !c2.phased) aff.push({ id: id, fields: { tapped: c2.tapped, attacking: !!c2.attacking } }); } return { t: "__set", cards: aff }; }
      case "__remove": { var rc = (a.ids || []).map(function (id) { return before.cards[id]; }).filter(Boolean).map(function (c) { return clone(c); }); return rc.length ? { t: "__add", cards: rc } : { t: "__noop" }; }
      case "__add": { return { t: "__remove", ids: (a.cards || []).map(function (c) { return c.instanceId; }) }; }
      ...
      case "commander_damage": { var cdp = before.players[a.seat]; return cdp ? { t: "__set", players: [{ seat: a.seat, fields: { cmdDamage: clone(cdp.cmdDamage || {}) } }] } : { t: "__noop" }; } // exact restore (delta-inverse breaks under the 0-clamp)
